...
public class E88_MergeSortedArray {
  public static void main(String[] args) {
    ...
    solution(nums1, 3, nums2, nums2.length);
  ...
  public static void solution(int[] nums1, int m, int[] nums2, int n) {
    ...
    while (index1 < m || index2 < n) {
      if((nums1[index1] != 0) && (nums1[index1] <= nums2[index2])) {
        System.out.println(nums2[index2]+" "+nums1[index1]);
        sorted[index++] = nums1[index1++];
      }
      else {
        System.out.println(nums2[index2]+"==");
